...
verify_all = function(A, A_m, tau, ranks, init = "svd", tol = 1e-12) {
  ...
  verify_norm_constraint(res_ns_mgcca_init, res_ns_mgcca_init$XtX)
  verify_orthogonality_constraints(res_ns_mgcca_init, res_ns_mgcca_init$XtX, tol = tol)
}

### Test ns_mgcca_init for matrix blocks
data(Russett)
...
test_that("Test that ns_mgcca_init generate a vector a that satisfies the norm constraint for 2D blocks", {
...
### Test ns_mgcca_init for tensor blocks
set.seed(0)
...
test_that("Test that ns_mgcca_init generate a vector a that satisfies the norm
