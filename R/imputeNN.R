...
imputeNN=function(A,k=1,output="mean", klim=NULL,scale=TRUE,sameBlockWeight=TRUE)
{
  ...
  # Each  variable is divided by its standard deviations, if scale =TRUE
   ...
   else{D=matrix(1,dim(superblockNA)[1],dim(superblockNA)[2])}
...
# 
# 
# superblockNA=do.call(cbind,listRes)
# group=unlist(lapply(listRes,"NCOL"))
# D=matrix(0,dim(superblockNA)[1],dim(superblockNA)[2])
# debutBlock=c(1,1+cumsum(group)[1:(length(group)-1)])
# finBlock=cumsum(group)
# for(k in 1:length(finBlock))
# {
#   D[,debutBlock[k]:finBlock[k]]=1/sqrt(group[k])
# }
# # on divise chaque colonne par son ecart type
# superblockNAs=scale(superblockNA)
# 
# # on divise par racine de p
# superblockNAs2=superblockNAs*D
