#' The function group_sgccak() is called by group_sgcca() and does not have to be
#' used by the user. group_sgccak() enables the computation of group SGCCA block
#' components, outer weight vectors, etc., for each block and each deflation stage.
...
group_sgccak <-  function(A, C, sparsity = rep(1, length(A)), group_sparsity, scheme = "centroid",
  ...
  var_order = lapply(group_sparsity, function (x) sort(x            = unlist(x),
                                                       decreasing   = FALSE,
                                                       index.return = TRUE*(!is.null(x)))$ix)
  ...
  for (q in 1:J){
    if (is.null(group_sparsity[[q]])){
      ...
      a[[q]] <- group_soft.threshold(a[[q]], const[q],
  ...
  if (iter > 1000) warning("The group SGCCA algorithm did not converge after 1000
                           iterations.")
    ...
    for (i in which(abs(l2_SAT - 1) > tol)){
      if (l2_SAT[i] < .Machine$double.eps ){
        ...
        warning("L2 constraint is not saturated for block #", i, ". The current
                ...
                be in the range [",
                sqrt(nMAX/(pjs[i]*as.numeric(nb_groups[i] == 0) + nb_groups[i])),
                ", 1] in order to saturate the L2 constraint.")
      }
    }
  }
