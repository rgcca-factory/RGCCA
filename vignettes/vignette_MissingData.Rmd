...
In this work, several solutions are investigated and compared on simulations and on multi-source biological data. Furthermore, a methodology for choosing the best solution for missing-data according to the dataset is given.  
...
The function createNA transforms a complete dataset into a dataset with missing values. The parameter pNA specifies the probability of missing values around the dataset (pNA is a proportion), by block (if pNA is a vector of length(A)) or by variable (if pNA is a list of length (A)). The minimum number of complete individuals can be specified in the option "nAllRespondants". Two options are available for the missing data: either they are missing by block (typeNA="block") or randomly in the block (typeNA="ponc")
```{r}
...
patternNA=determine_patternNA(blocks=A_missing)
names(patternNA)
```

```{r, fig.height = 3, fig.width = 7, fig.align = "center"}
A_missing2=createNA(blocks=A,typeNA="byvar",pNA=list(0.1*1:3,rep(0,2),rep(0.1,3)))$dat
...
rgcca_nipals=rgcca(blocks =A_missing,connection =C,tau=rep(1,length(A)),verbose=FALSE,method="nipals")
rgcca_nipals2=rgccad(A=A_missing,C=C,tau=rep(1,length(A)),verbose=FALSE)
...
plot_MIRGCCA(mi.obj=mi_rgcca,multiple="ell",indnames=FALSE)
```
```{r, fig.height = 5, fig.width = 5, fig.align = "center"}
plot_MIRGCCA(mi.obj=mi_rgcca,multiple="seg",indnames=FALSE)
...
whichNAmethod simulates nDatasets (20 by default) with missing values according to a specific pattern of missing values (patternNA) and typeNA ("block","byvar" or "ponc"). plotWhichNAmethod allows to select one of the following indicators
...
listResults=naEvolution(blocks=A,listMethods=c("complete","nipals","mean"),prctNA=c(0.05,0.1,0.15,0.2,0.25,0.3,0.4),typeNA="ponc",ncomp=rep(1,3),sameBlockWeight=FALSE,nDatasets = 10,connection=C)
```
