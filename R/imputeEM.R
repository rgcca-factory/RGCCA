...
imputeEM <-
    function(A,
        ...
        # to get the superblock concatenedBlocks', with missing values imputed
        # by the colmeans
        concatenedBlocks <- Reduce(cbind, Alist)
        
        
        # Normalize the blocks
        scaledConcatenedBlocks <-
            scale2(concatenedBlocks, scale = scale, bias = FALSE)
        ...
        if (superblock) {
            ...
            C2[J + 1, J + 1] <- 0
            tau2 <-
                c(tau, 0)  # mode A for the blocks and mode B for superblock (design for CPCA2 )
        }
        while (continue) {
            diff[[i]] <-
                objective[[i]] <- old[[i + 1]] <- criterion[[i]] <- list()
            ...
            if (superblock) {
                ...
                if (naxis == 1) {
                    ...
                    if (reg == "w") {
                        w1 <- w[, 1]
                        gamma <-
                            apply(scaledConcatenedBlocks, 1, function(x)
                                lm(x ~
                                       0 + w)$coefficients[1])
                        # =t(as.matrix(w))%*%t(scaledConcatenedBlocks)/sum(w*w) sigma est la
                        # somme des residus
                        residuals <-
                            apply(scaledConcatenedBlocks, 1, function(x)
                                (lm(x ~
                    ...
                    if (reg == "y") {
                        w1 <- w[, 1]
                        gamma <-
                            apply(scaledConcatenedBlocks, 2, function(x)
                                lm(x ~
                                       0 + y)$coefficients[1])
                        residuals <-
                            apply(scaledConcatenedBlocks, 2, function(x)
                        ...
                        sigma <- sqrt(sum(residuals ^ 2 / (J * nsuj)))
                        centeredXhat <-
                            matrix(y, ncol = naxis) %*% matrix(gamma,
                                                               nrow = naxis)
                        # if(noise){eps=matrix(rnorm(dim(moy)[1]*dim(moy)[2],m=0,sd=sigma),nrow=dim(moy)[1],ncol=dim(moy)[2])}else{eps=0}
                    ...
                    if (reg == "no") {
                        w1 <- w[, 1]
                        # gamma=apply(scaledConcatenedBlocks, 2, function(x)
                        # lm(x~0+y)$coefficients[1])
                        residuals <- 0
                        sigma <- sqrt(sum(residuals ^ 2 / (J * nsuj)))
                        centeredXhat <-
                            matrix(y, ncol = naxis) %*% matrix(w,
                                                               nrow = naxis)
                        # if(noise){eps=matrix(rnorm(dim(moy)[1]*dim(moy)[2],m=0,sd=sigma),nrow=dim(moy)[1],ncol=dim(moy)[2])}else{eps=0}
                ...
                if (naxis > 1) {
                    ...
                    for (k in 1:naxis) {
                        ...
                        gamma <-
                            rbind(
                                gamma,
                                apply(scaledConcatenedBlocks, 2,
                                      function(x)
                                          lm(x ~ 0 + y[, k])$coefficients[1])
                            )
                    }
                    centeredXhat <-
                        matrix(y, ncol = naxis) %*% matrix(gamma,
            ...
            if (!superblock) {
                ...
                for (j in 1:J) {
                    ...
                    if (naxis == 1) {
                        ...
                        if (reg == "w") {
                            gamma <- apply(scaledConcatenedBlocks[, debutBlock[j]:finBlock[j]],
                                           1, function(x)
                                               lm(x ~ 0 + w)$coefficients[1])
                            residuals <-
                                apply(scaledConcatenedBlocks[, debutBlock[j]:finBlock[j]],
                                      1, function(x)
                                          (lm(x ~ 0 + w)$residuals))
                            sigma[[j]] <-
                                sqrt(sum(residuals ^ 2 / (J * nsuj)))
                            centeredXhat[[j]] <-
                                matrix(gamma, ncol = naxis) %*%
                                matrix(w, nrow = naxis)
                    ...
                    if (naxis > 1) {
                        ...
                        for (k in 1:naxis) {
                            ...
                            gamma <-
                                rbind(
                                    gamma,
                                    apply(scaledConcatenedBlocks[,
                                                                 debutBlock[j]:finBlock[j]], 2, function(x)
                                                                     lm(x ~
                                                                            y[, k])$coefficients[2])
                                )
                            # residuals=cbind(residuals,apply(scaledConcatenedBlocks, 1,
                            # function(x) lm(x~w[,k])$residuals))
                        }
                        centeredXhat[[j]] <- y %*% gamma
                        residuals <-
                            scaledConcatenedBlocks[, debutBlock[j]:finBlock[j]] -
                            centeredXhat[[j]]
            ...
            # concatened blocks is imputed by concatenedXhat and Alist,
            # concatenedBlocks and scaledConcatenedBlock are reaffected
            concatenedBlocks[indNA] <- concatenedXhat[indNA]
            scaledConcatenedBlocks <-
                scale2(concatenedBlocks, scale = scale,
                       bias = FALSE)
