...
rownames(X1)=rownames(X2)=paste("S",1:70)
 A=list(X1,X2)
 listResults=naEvolution(A=A,prctNA=c(0.1,0.2),listMethods=c("mean","complete","nipals","em","sem1","knn4"),nDatasets = 1)
 
 
 listResults=naEvolution(A=A,prctNA=c(0.1,0.2),listMethods=c("knn4","knn5"),nDatasets = 1)
