...
tsgcca <- function(A, C = 1-diag(length(A)), sparsity = rep(1, length(A)),
                  ncomp = rep(1, length(A)), scheme = "centroid", scale = TRUE,
                  init="svd", bias = TRUE, tol = 1e-8, verbose=FALSE,
                  scale_block = TRUE, ranks = rep(1, length(A)),
  ...
  if (mode(scheme) != "function") {
    ...
    if (verbose) cat("Computation of the TSGCCA block components based on the",
                     scheme, "scheme \n")
  }
  if (mode(scheme) == "function" & verbose) {
    cat("Computation of the TSGCCA block components based on the g scheme \n")
  }
  ...
  if (is.numeric(sparsity))
  {
    sparsity_mat = sparsity
    if(is.vector(sparsity_mat)) {
      sparsity_mat = matrix(sparsity_mat, nrow = max(ncomp), ncol = J, byrow = TRUE)
    }
    if(is.matrix(sparsity_mat)) colnames(sparsity_mat) = names(A)
  }
  else
  {
    sparsity_mat = matrix(NA, max(ncomp), J)
    colnames(sparsity_mat) = names(A)
  }

  ##########################################
  ### Determination of TSGCCA components ###
  ##########################################
  for (n in 1:(N+1)) {
    if (verbose) cat(paste0("Computation of the TSGCCA block components #", n,
                            " is under progress...\n"))
    # n_random_starts
    tsgcca.result = tsgccak(
      A                       = R,
      A_m                     = R_m,
      C                       = C,
      sparsity                = sparsity_mat[n, ],
      scheme                  = scheme,
      init                    = init,
      bias                    = bias,
      tol                     = tol,
    ...
    # Store sparsity, AVE_inner, crit
    AVE_inner[n] = tsgcca.result$AVE_inner
    crit[[n]]    = tsgcca.result$crit

    # Store Y, a, factors and weights
    for (d in 1:J) Y[[d]][,n] = tsgcca.result$Y[ , d]
    for (d in 1:J) a[[d]][,n] = tsgcca.result$a[[d]]
    for (d in B_nD) {
      for (f in 1:(LEN[[d]] - 1)) {
        idx                      = seq((n - 1) * ranks[[d]] + 1, n * ranks[[d]])
        factors[[d]][[f]][, idx] = tsgcca.result$factors[[d]][[f]]
      }
      weights[[d]] = tsgcca.result$weights[[d]]
    }

    # Deflation procedure
    defla.result = defl.select(tsgcca.result$Y, R, ndefl, n, nbloc = J)
    ...
    for (d in 1:J){
      if (n == 1){
        astar[[d]][,n] = tsgcca.result$a[[d]]
      }else{
        astar[[d]][,n] = tsgcca.result$a[[d]] - astar[[d]][,1:(n-1), drop=F] %*%
