...
#' @examples 
#'  data();...
imputeEM=function(A,C,tau,ni=50,tol=1e-8,graph=FALSE,ncomp=NULL,naxis=1,scale=TRUE,sameBlockWeight=TRUE,scheme="centroid",bias=TRUE,superblock=FALSE,noise=FALSE)
{
    #listWithoutNA
  nvar=sapply(A,NCOL)
  nsuj=dim(A[[1]])[1]
  ...
  indNA2=lapply(A,function(x){return(which(is.na(x),arr.ind=TRUE))})
  # Imputing Alist by the mean
   ...
   # Calculating stats and ends of each block in the superbblock
