gsgcca_penalized = function(blocks, connection = 1 - diag(length(blocks)),
                            sparsity = rep(1, length(blocks)),
                            ncomp = rep(1, length(blocks)), scheme = "centroid",
                            init = "svd", bias = TRUE, tol = 1e-08, verbose = TRUE,
                            na.rm = TRUE, quiet = FALSE, penalty_coef = 0)
{

  if (mode(scheme) != "function") {
    if (verbose)
      cat("Computation of the gSGCCA_penalized block components based on the",
          scheme, "scheme \n")
  }
  if (mode(scheme) == "function" & verbose)
    cat("Computation of the gSGCCA_penalized block components based on the g scheme \n")
  ...
  gcca.result <- gsgccak_penalized(blocks, connection, sparsity = sparsity, scheme = scheme,
