    ...
    orth_Y = TRUE)
{
    ...
    if (any(sapply(blocks, function(x) length(dim(x))) > 2)) {
        if(!type %in% c("mgcca", "ns_mgcca", "gmgcca", "ns_mgcca_penalized",
                        ...
                        "tgcca", "gmgcca_pdd_nuclear", "gmgcca_pdd"))
        {
    ...
    } else if (tolower(type) %in% c("gmgcca_aux_y")) {
      gcca <- gmgcca_aux_YNa
      par <- "tau"
      penalty <- tau
    ...
    } else if (tolower(type) %in% c("gmgcca_pdd")) {
      gcca <- gmgcca_pddNa
      par <- "tau"
      penalty <- tau
  ...
  # Check blocks size, add NA for missing subjects
   ...
   if(!is.null(response))
   {
    ...
    if(type == "gmgcca_aux_y") func_out$call$ranks = ranks
    if(type == "gmgcca_aux_var") func_out$call$ranks = ranks
