...
extract the information shared by the $J$ random composite variables, taking into account an undirected graph of connections between them. 
...
itself is multi-convex (i.e. for each $j$, $f$ is a convex function of 
$\mathbf a_j$ while all the other $\mathbf a_k$ are fixed) continuously differentiable. When at least one element 
of the diagonal of $\mathbf{C}$ is different from $0$, additional conditions have 
to be imposed on g to keep $f$ multi-convex. For example, when g is 
...
general optimization framework presented in the next section.

## Algorithm

The RGCCA framework relies on a master algorithm for maximizing a continuously 
differentiable multi-convex function 
$f( \mathbf a_1, \ldots, \mathbf a_J):\mathbb{R}^{p_1}\times \ldots \times \mathbb{R}^{p_J} \xrightarrow{}\mathbb{R}$ under the constraint that 
...
centered. The optimization problem considered within the RGCCA package is:
...
$\mathbf M_j = \widehat{\mathbf{\Sigma}}_{jj}$ is an 
