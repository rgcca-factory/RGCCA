...
Different functions are then implemented to automatically choose some of the parameters of the $\small{\texttt{rgcca()}}$ function, to evaluate the weights found by the method, make predictions, or provide visualizations.
...
A bootstrap resampling method can be used to assess the reliability of parameter estimates (block-weight/loading vectors) obtained using RGCCA. $\small{\texttt{B = n\_boot}}$ bootstrap 
...
function is available using the $\small{\texttt{rgcca\_bootstrap()}}$ function of the $\small{\texttt{RGCCA}}$ 
package.
...
proposed in \cite{Witten2009a} has also been integrated within the $\small{\texttt{RGCCA}}$ package 
...
<!-- ### Stability selection -->
**Stability selection**.
It is possible to stabilize the selected variables using 
