...
fit.rgcca = rgcca(A, connection = C, tau = c(0.7, 0.8, 0.7),
...
test_that("rgcca_transform raises an error if block names do not match", {
  expect_error(rgcca_transform(fit.rgcca, list("wrong_name" = 42)),
               paste0("At least one block from X was not found in the training",
...
test_that("rgcca_transform raises an error if block names do not match", {
  expect_error(rgcca_transform(fit.rgcca, list("agriculture" = 42)),
               paste0("Dimensions of blocks do not match for block",
                      "agriculture"),
               fixed = TRUE)
})
...
fit.rgccaNA = rgcca(blocksNA, connection = C, tau = c(0.7,0.8,0.7),
