...
res2=rgcca_cv(blocks,response=length(blocks), type="rgcca",par_type="tau",par_value=c(0,0.2,0.3),n_run=5,n_cores=1)
plot(res)
...
res5=rgcca_cv(blocks,response=length(blocks), type="rgcca",par_type="sparsity",par_value=matrix(c(0.8,0.9,1,0.9,1,1),nrow=2,byrow = T),n_run=1, one_value_per_cv = FALSE,n_cores=1)
...
res=rgcca_cv(blocks_for_classif,response=3, type="rgcca",par_type="tau",par_value=c(0,0.2,0.3),ncomp=1,n_run=1,n_cores=1,type_cv="classification",fit="lda")
