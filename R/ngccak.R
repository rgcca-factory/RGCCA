#' The function ngccak() is called by ngcca() and does not have to be used by
#' the user. ngccak() enables the computation of NGCCA block components, outer
...
#' @title Internal function for computing the NGCCA parameters (NGCCA block
#' components, outer weight vectors etc.)
#' @importFrom Deriv Deriv
ngccak <-  function(A, C, scheme = "centroid",
  ...
  if (iter > n_iter_max) {
    stop_rgcca("The NGCCA algorithm did not converge after ", n_iter_max,
               " iterations.")}

  if (iter < n_iter_max & verbose) {
    message("The NGCCA algorithm converged to a stationary
