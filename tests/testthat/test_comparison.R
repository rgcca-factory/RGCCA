...
# 
# refRgcca=rgccad(A, C = 1 - diag(length(A)), tau = rep(1, length(A)), ncomp = rep(2, length(A)), scheme = "centroid", scale = TRUE,   init = "svd", bias = TRUE, tol = 1e-08, verbose = TRUE,sameBlockWeight=TRUE,na.rm=FALSE)
...
# methodRgcca=rgccaNa(A=listNAdataset$dat,method="em",verbose=FALSE,ncomp=rep(2,length(A)))
# resComp=NULL
