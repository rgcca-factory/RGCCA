# In this version, we add a constraint on the nuclear norm of the component
# To make it work, we write Y as Y = PD with D diagonal and we constrain
# the nuclear norm of P
# We also separate the criterion between a and Y
gmgccak_PDD_nuclear3 <- function(A, C, tau = rep(1, length(A)), scheme = "centroid",
  ...
  criterion <- function() {
    ...
    for (i in 1:J) {
      for (j in 1:J) {
        cur_crit <- c(cur_crit, C[i, j] * sum(diag(g(crossprod(Y[[i]], Z[[j]])))))
      }
  ...
  Z_update <- function(j) {
    ...
    # Update D
    new_P <- U %*% diag(s, nrow = ncol(Q)) %*% t(V)
    dd <- diag(crossprod(new_P))
    x <- diag(t(Q) %*% new_P)
    # Test norm of d, if it's below lambda x the number of components, the
    # Lagrange multiplier associated to the norm constraint is null
    if (drop(t(x) %*% diag(1 / dd^2, nrow = ncol(Q)) %*% x) <= ncol(Q) * lambda[j]) {
      new_D <- x / dd
    } else {
      val <- lagrange_search(dd, x, ncol(Q) * lambda[j])
      new_D <- x / (dd + val)
    }
  ...
  for (j in seq(J)) {
    # SVD <- svd(Y[[j]])
    SVD <- svd(matrix(rnorm(prod(dim(Y[[j]]))), nrow = n))
    P[[j]] <- SVD$u %*% t(SVD$v)
    # D[[j]] <- diag(SVD$d, nrow = ncol(Y[[j]]))
    D[[j]] <- diag(sqrt(diag(crossprod(Y[[j]]))), nrow = ncol(Y[[j]]))
    Z[[j]] <- P[[j]] %*% D[[j]]
  }
