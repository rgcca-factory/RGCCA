...
#' @noRd
rgccak_mg <- function(A, C, tau = rep(1, length(A)), scheme = "centroid",
  ...
  ### Initialization
  init_object <- ifelse(is.null(groups),
                        yes = rgcca_init(A, init, bias, na.rm, tau),
                        no = rgcca_init_mg(A, init, bias, na.rm, tau, groups)) #remove bias?
  a <- init_object$a
  Y <- init_object$Y
