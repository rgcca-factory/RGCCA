...
bootstrap_k <- function(rgcca_res, type = "weight") {
    ...
    if (type == "weight") {
        ...
        w <- lapply(1:length(rgcca_res_boot$blocks), function(j) {
            res <- sapply(1:dim(rgcca_res_boot$blocks[[j]])[2], function(k) {
                cor(
                    rgcca_res_boot$Y[[j]][, 1], 
                    rgcca_res_boot$blocks[[j]][, k], 
                    use = "pairwise.complete.obs")
            })
            names(res) <- colnames(rgcca_res_boot$blocks[[j]])
            return(res)
        })
    }
}
