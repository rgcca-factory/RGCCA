...
Regularized Generalized Canonical Correlation Analysis (RGCCA) \cite{Tenenhaus2011, Tenenhaus2014, Tenenhaus2015, Tenenhaus2017} is a unified statistical framework that gathers six decades of multiblock component methods.
...
have come to the fore for carrying out multiblock data analysis. 
The bioinformatician community mostly uses $\small{\texttt{mixOmics}}$ \cite{Rohart2017} which wraps the main functions of the $\small{\texttt{RGCCA}}$ package for performing multiblock analyses.

The $\small{\texttt{ade4}}$ package \cite{Dray2007} cover a wide range of multivariate methods. its implementation follows the tradition of the French school of "Analyse des Donnees" and is based on the use of the duality diagram. $\small{\texttt{ade4}}$ mainly relies on three approaches for performing multiblock analysis: Multiple co-inertia analysis (MCOA) \cite{Chessel1996}, Multiple Factor Analysis (MFA) \cite{Escofier1994} and Statis \cite{Lavit1994}.
...
The $\small{\texttt{multiblock}}$ package \cite{Liland2022} covers a wide range of multiblock methods for data fusion, but intensively relies on a wrapper strategy for performing multiblock analyses. Consequently, this results in strong dependencies  between the $\small{\texttt{multiblock}}$ package and other packages including (i) FactoMineR (for MFA and GPA), RGCCA (for Hierarchical PCA \citep{Wold1996, Hanafi2010}, MCOA, Interbattery Factor Analysis \citep{Tucker1958} and Carroll's GCCA \cite{Carroll1968a}), ade4 (Multiblock redundancy analysis \cite{Bougeard2011} and statis), r.jive (for JIVE \cite{Lock2013}), RegularizedSCA (for DISCO \cite{Schouteden2014}).
...
Within the $\small{\texttt{RGCCA}}$ package, all implemented methods are made available 
through the single $\small{\texttt{rgcca()}}$ function and thus share the same function 
interface and a clear class structure. In addition to the main statistical functionalities, the $\small{\texttt{RGCCA}}$ package provides several 
utility functions for data preprocessing and offers various advanced ggplot visualization tools to help users interpret and extract meaningful information from integrated data. It also includes metrics for assessing the robustness and significance of the analysis.
The package also includes several built-in datasets and examples to 
...
In this section we briefly present RGCCA \cite{Tenenhaus2011, Tenenhaus2015, Tenenhaus2017} which constitutes the core of the RGCCA framework.  

A sample-based optimization problem related to \eqref{optim_RGCCA} can defined by considering $n$ observations of the vector $\boldsymbol x$. It yields a column-partition data matrix  $\mathbf X = [\mathbf X_1, \ldots, \mathbf X_j, \ldots, \mathbf X_J] \in \mathbb{R}^{n \times p}$. Each $n\times p_j$ data matrix $\mathbf X_j$ is called a block and represents a set of $p_j$ variables observed on the same set of $n$ individuals. The variables’ number and nature may differ from one block to another, but the individuals must be the same across blocks. We assume that all variables are centered.
...
We now present the usefulness and versatility of the RGCCA package on two real datasets.

## Software design

We now present the functions implemented in the $\small{\texttt{RGCCA}}$ package. 
The list of functions can be found in Table \ref{exported_functions}.
