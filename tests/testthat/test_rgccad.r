...
load(file = "../results/resultRgcca_Tau1")
load(file = "../results/resultRgcca_Tau0")
load(file = "../results/resultRgcca_TauOpt")
data(Russett)
...
T1 <- Sys.time()
resultRgcca_Tau1_test <- rgccad(A, C, ncomp = rep(2, 3), tau = c(1, 1, 1), scheme = "factorial", verbose = FALSE)
T2 <- Sys.time()
Tdiff_Tau1_test <- T2 - T1
# 0.03056502
T1 <- Sys.time()
resultRgcca_Tau0_test <- rgccad(A, C, ncomp = rep(2, 3), tau = c(0, 0, 0), scheme = "factorial", verbose = FALSE)
T2 <- Sys.time()
Tdiff_Tau0_test <- T2 - T1
# 0.03978729
T1 <- Sys.time()
resultRgcca_TauOpt_test <- rgccad(A, C, ncomp = rep(2, 3), tau = rep("optimal", 3), scheme = "factorial", verbose = FALSE)
T2 <- Sys.time()
Tdiff_TauOpt_test <- T2 - T1
# 0.04306483   secs
...
plot(result.rgcca$Y[[1]], result.rgcca$Y[[2]],
  col = "white",
