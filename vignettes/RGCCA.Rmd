...
abstract: 
    ...
    relationships between multiple sets of variables. Regularized Generalized 
    Canonical Correlation Analysis (RGCCA) is a unified and versatile framework 
    that consolidates over six decades of multiblock component methods. The 
    $\small{\texttt{RGCCA}}$ package offers an implementation for this framework.
    Beyond implementation, the $\small{\texttt{RGCCA}}$ package proposes 
    graphical outputs and statistics to assess the robustness/significance of 
    the analysis. The usefulness of the $\small{\texttt{RGCCA}}$ package is 
    illustrated in this paper on two real datasets. The $\small{\texttt{RGCCA}}$ 
    package is freely available on the ComprehensiveR Archive Network (CRAN) 
    http://www.r-project.org/ and GitHub https://github.com/rgcca-factory/RGCCA.
keywords:
...
Usually, $c_{jk} = 1$ between two connected blocks and $0$ otherwise. 
\end{itemize}
...
optimization problem (\ref{optim_RGCCA_sample2}). These multiblock component methods are 
...
the $\small{\texttt{RGCCA}}$ package are reported below: 
...
$p_j \times p_j$. A dual formulation described in \cite{Tenenhaus2015} requires handling matrices of dimension $n \times n$ . Therefore, the primal 
formulation of the RGCCA algorithm will be preferred when $n>p_j$, and the dual form will be used when $n \le p_j$.  The $\small{\texttt{rgcca()}}$ function of the $\small{\texttt{RGCCA}}$ package implements these two formulations and automatically selects the best one.  
...
vectors $\mathbf a_1 , \ldots ,  \mathbf a_{j-1} ,  \mathbf a_{j+1} , \ldots ,  \mathbf a_J$ are fixed. 
...
of the sequence $\left\lbrace  \mathbf a^s \right\rbrace$. The full proof of these properties is given in \cite{Tenenhaus2017}.
...
$\ell_1$-sphere of radius $s_j$ necessarily intersect. Within the $\small{\texttt{RGCCA}}$ package, 
...
The usefulness and versatility of the $\small{\texttt{RGCCA}}$ package are illustrated in the next 
section.
...
the $\small{\texttt{RGCCA}}$ package. The Russett dataset \citep{Russett1964} is studied in \cite{Gifi1990}. 
...
($\small{\texttt{superblock = TRUE}}$ or $\small{\texttt{method}}$ set to a method that induces the use of superblock), global components can be derived. The space spanned by the global 
...
**Choice of the shrinkage parameters**.
In the previous section, the shrinkage parameters were manually set. However,  
the $\small{\texttt{RGCCA}}$ package introduces three fully 
automated strategies for selecting the optimal shrinkage 
parameters.
...
The fitted permutation object, $\small{\texttt{perm\_out}}$, can be directly 
provided as the output of $\small{\texttt{rgcca()}}$ and visualized/bootstrapped 
as usual.
...
obtained by cross-validation. We will illustrate this strategy in the second case study,
in the forthcoming section.

<!-- ### RGCCA with superblock -->
**RGCCA with a superblock**.
To conclude this section on the analysis of the Russett dataset, we consider 
Multiple Co-Inertia Analysis \citep{Chessel1996} \citep[MCOA, also called MCIA in][]{Cantini2021} with $2$ components per block. 
...
$\small{\texttt{sparsity}}$ argument of $\small{\texttt{rgcca()}}$ varies 
between 1/sqrt(ncol) and 1 (larger values of $\small{\texttt{sparsity}}$ 
correspond to less penalization) and controls the amount of 
...
argument $\small{\texttt{method = "sgcca"}}$ or by specifying directly the $\small{\texttt{sparisty}}$ argument as below. 
...
This is made possible by setting the $\small{\texttt{par\_type}}$ argument to $\small{\texttt{"sparsity"}}$ (instead of $\small{\texttt{"tau"}}$) within the $\small{\texttt{rgcca\_permutation()}}$ function. However, in this section, we 
will adopt a different approach. 

The optimal tuning parameters can be determined 
by cross-validating using different indicators of quality, namely:
...
cross-validated balanced accuracy ($\small{\texttt{metric = "Balanced\_Accuracy"}}$) in a model where we try to predict the response block from all the block components with a user-defined classifier ($\small{\texttt{prediction\_model = "lda"}}$). The cross-validation protocol is set by specifying the arguments the number of folds ($\small{\texttt{k}}$) and the number of cross-validations to be run ($\small{\texttt{n\_run}}$). Also, we decide to upper bound the sparsity parameters for $\mathbf X_1$ and $\mathbf X_2$ to $0.2$ to achieve an 
...
offers a unified implementation strategy for these methods. The $\small{\texttt{RGCCA}}$ package 
is available on the Comprehensive R Archive Network (CRAN) and GitHub 
https://github.com/rgcca-factory/RGCCA. This release of the $\small{\texttt{RGCCA}}$ package 
includes:
...
We believe that the $\small{\texttt{RGCCA}}$ package will be a valuable resource for researchers 
...
next release of the $\small{\texttt{RGCCA}}$ package.
