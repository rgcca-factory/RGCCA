...
# on the other side
# In this version, we impose constraints on each element of delta instead of
# constraining its norm.
gmgccak_PDD6 <- function(A, C, tau = rep(1, length(A)), scheme = "centroid",
  ...
  Z_update <- function(j) {
    ...
    if (orth_Y) {
      ...
      delta[delta < -sqrt(lambda_max[j])] <- -sqrt(lambda_max[j])
      delta[delta > -sqrt(lambda_min[j]) & delta < 0] <- -sqrt(lambda_min[j])
      delta[delta < sqrt(lambda_min[j]) & delta > 0] <- sqrt(lambda_min[j])
      delta[delta > sqrt(lambda_max[j])] <- sqrt(lambda_max[j])
  ...
  eigens <- lapply(A_m, function(x) {
    if (nrow(x) > ncol(x)) {
      eigen(crossprod(x), symmetric = TRUE, only.values = TRUE)$values
    } else {
      eigen(tcrossprod(x), symmetric = TRUE, only.values = TRUE)$values
    }
  })

  lambda_max <- vapply(eigens, "[", 1, FUN.VALUE = 1.)
  lambda_min <- vapply(eigens, function(x) {
    idx <- x > 1e-10
    if (sum(idx) == 0) {
      return(x[1])
    }
  ...
  if (orth_Y) {
    ...
    # # Standard deviation such that E[Mu[[j]][, h]] = lambda_max[j]
    # Mu <- lapply(seq(J), function(j) matrix(
    #   rnorm(n * ncomp[j], sd = sqrt(lambda_max[j] / n)), nrow = n, ncol = ncomp[j]
    # ))
    # Standard deviation such that E[Mu[[j]][, h]] = lambda_max[j]
    Mu <- lapply(seq(J), function(j) vapply(seq(ncomp[j]), function(h) {
      rnorm(n, sd = norm(Y[[j]][, h], type = "2") / sqrt(n))
    }, FUN.VALUE = double(n)) / rho)
  } else {
    Mu <- lapply(seq(J), function(j) matrix(0, nrow = pjs[j], ncol = ncomp[j]) / rho)
  }
